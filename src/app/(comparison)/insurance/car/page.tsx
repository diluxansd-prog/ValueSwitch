...
function CarInsuranceContent() {
  ...
  const fetchResults = useCallback(async () => {
    ...
    try {
      ...
      if (filters.coverLevel) queryParams.set("coverLevel", filters.coverLevel);
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-white/80">
                Find the cheapest car insurance quotes from top UK providers
              </p>
            </div>
            {results && (
              <div className="text-sm font-medium text-white/90">
                {results.totalCount} quotes found
              </div>
            )}
          </div>
        </div>
          ...
          <div className="flex items-center gap-3">
            <Sheet open={mobileFiltersOpen} onOpenChange={setMobileFiltersOpen}>
              ...
              <SheetContent side="left" className="w-80 overflow-y-auto">
                ...
                <div className="mt-6">
                  <ComparisonFilters category="insurance" onFiltersChange={handleFiltersChange} providers={providers} />
                </div>
              </SheetContent>
            </Sheet>
              ...
              {loading ? (
                <div className={viewMode === "grid" ? "grid grid-cols-1 gap-6 md:grid-cols-2" : "flex flex-col gap-4"}>
                  {Array.from({ length: 6 }).map((_, i) => (
                    <div key={i} className="rounded-xl border bg-background p-6">
                      ...
                      <div className="grid grid-cols-2 gap-2 mb-4"><Skeleton className="h-12 rounded-lg" /><Skeleton className="h-12 rounded-lg" /></div>
                ...
                <>
                  <p className="mb-6 text-sm text-muted-foreground">
                    Showing {Math.min((page - 1) * ITEMS_PER_PAGE + 1, results.totalCount)}-{Math.min(page * ITEMS_PER_PAGE, results.totalCount)} of {results.totalCount} car insurance quotes
                  </p>
                  <div className={viewMode === "grid" ? "grid grid-cols-1 gap-6 md:grid-cols-2" : "flex flex-col gap-4"}>
                    ...
                    <div className="mt-10 flex items-center justify-center gap-2">
                      ...
                      <div className="flex items-center gap-1">
                        {Array.from({ length: Math.min(totalPages, 5) }).map((_, i) => {
                          const pageNum = i + 1;
                          return (<Button key={pageNum} variant={page === pageNum ? "default" : "outline"} size="sm" className="size-9" onClick={() => setPage(pageNum)}>{pageNum}</Button>);
                        })}
                        {totalPages > 5 && (<><span className="px-1 text-muted-foreground">...</span><Button variant={page === totalPages ? "default" : "outline"} size="sm" className="size-9" onClick={() => setPage(totalPages)}>{totalPages}</Button></>)}
                      </div>
                ...
                <div className="flex flex-col items-center justify-center rounded-xl border bg-background p-12 text-center">
                  <Car className="mb-4 size-12 text-muted-foreground/50" />
                  <h3 className="mb-2 text-lg font-semibold">No car insurance quotes found</h3>
                  <p className="mb-6 max-w-md text-sm text-muted-foreground">Try adjusting your filters or broadening your search.</p>
